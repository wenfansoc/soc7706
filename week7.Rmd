...
For log-normal models, both `survreg()` and `flexsurvreg()` use the AFT parameterization.
...
For log-logistic models, both `survreg()` and `flexsurvreg()` use the AFT parameterization.
...
Gompertz models are not available in `survreg()`, but can be fitted using `flexsurvreg()`, which models the hazard rate.
