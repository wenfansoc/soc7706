...
The PSID data are currently in long format, where each row represents an observation for a person in a specific year. We can reshape it to wide format if needed, but for fixed-effects models, we typically work with long format.

```{r reshape}
# Long to wide
psid_wide <- psid |>
  pivot_wider(id_cols = id, names_from = year, values_from = c(yrsch, srh, pincome1, hwsei), names_sep = "_")

# Wide to long
psid_long <- psid_wide |>
  pivot_longer(cols = starts_with(c("yrsch_", "srh_", "pincome1_", "hwsei_")),
...
within_variation <- psid |>
  group_by(id) |>
  summarize(
    yrsch_sd = sd(yrsch, na.rm = TRUE),
    srh_sd = sd(srh, na.rm = TRUE),
    pincome1_sd = sd(pincome1, na.rm = TRUE),
...
between <- psid |>
  ...
  summarize(
    ...
    pincome1_mean = mean(pincome1, na.rm = TRUE),
    .groups = 'drop'
  )
...
within <- psid |>
  ...
  group_by(id) |>
  mutate(
    yrsch_ch = yrsch - dplyr::lag(yrsch, 1), # Make sure to add dplyr:: before lag to avoid confusion with lag from other packages
    pincome1_ch = pincome1 - dplyr::lag(pincome1, 1)
  ) |>
...
Let’s compare fixed-effects estimates from the within transformation and first differences in the two-wave case.
